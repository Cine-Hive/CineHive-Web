...
export default {
  ...
  methods: {
    async fetchLikedReplies() {
      const token = localStorage.getItem('token');
      try {
        const response = await axios.get('http://localhost:8081/myPage/reply-likes', {
          headers: { Authorization: `Bearer ${token}` }
        });
        this.replies = response.data;
      } catch (error) {
        console.error('좋아요한 감상평 조회 실패:', error);
      } finally {
        this.loading = false;
      }
    },
